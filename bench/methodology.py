...
Pre-registered hypotheses (bench/hypotheses.json), multicollinearity
...
built by bench/baseline_corpus.py; nothing here trusts a file that
has not been re-hashed against its manifest.

Outputs:
  - backend/artifacts/methodology-report.json  (canonical SHA-256 signed)
  - backend/artifacts/methodology-report.md    (optional human-readable rendering)

Run from the repository root:  python -m bench.methodology
"""
...
REPORT_MD = ROOT / "backend" / "artifacts" / "methodology-report.md"
SEED = 13
PERMUTATIONS = 2000
BOOTSTRAP = 2000
