...
def clean(tarball: Path, clean_path: Path) -> dict:
    ...
    n_files = 0
    for family_group, domain, version, payload, rel in _iter_raw_data(tarball):
        n_files += 1
        originals = payload.get("original") or []
        sampled = payload.get("sampled") or []
        for index, (human_text, ai_text) in enumerate(zip(originals, sampled, strict=False)):
...
def pointer_manifest(manifest: dict) -> dict:
    payload = {
        ...
        "id": "evobench",
        "kind": "prose",
        "title": "EvoBench — detector generalization across evolving LLM versions (ACL 2025 Findings)",
        "license": {
            "spdx": "NOASSERTION",
        ...
        "splits": {
            "train": {"groups": ["domain:sha256(human original)[:16]"], "n": manifest["splits"]["clean"]["rows_clean"]},
            "calibration": {"groups": ["domain:sha256(human original)[:16]"], "n": manifest["splits"]["clean"]["rows_clean"]},
            "test": {"groups": ["domain:sha256(human original)[:16]"], "n": manifest["splits"]["clean"]["rows_clean"]},
            "group_keys": ["pair"],
        },
        "labels": {
            "schema": "binary_ai",
        ...
        "limitations": [
            "The 'harmful' domain is paraphrased social-media content; label semantics differ from the continuation domains.",
            "Human originals are shared across LLM versions within a domain; pair groups keep them out of different folds.",
...
def main() -> int:
    ...
    if args.check:
        ...
        if not clean_path.exists() or sha256_file(clean_path) != manifest["splits"]["clean"]["clean_sha256"]:
