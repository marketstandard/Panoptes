...
This is the procedure an **outside researcher** should follow. The authors' own run of `python research/reproduce.py` is a first-party self-check (`independent: false` in `backend/artifacts/reproduction-selfcheck.json`). It does not satisfy RQ4.
...
```bash
python research/fetch_defactify.py --check
...
Open a pull request that adds `research/submissions/<your-lab>-reproduction/` with the filled table and the SHA of the commit you checked out. Do not commit raw Defactify text.
