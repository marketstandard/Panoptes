...
A detector score is not a probability. Panoptes maps raw detector scores into calibrated outcomes through an isotonic-regression artifact fitted on the verified reference corpus (`backend/artifacts/baseline-calibration.json`, regenerated by `python research/calibration.py`). The calibrated probability is converted to a prevalence-corrected likelihood ratio, then combined with the user-declared prior. ECE is reported as a diagnostic of that map, not mixed into the posterior. The calibration bundle ID, cohort, cohort prevalence, reliability error, and corpus provenance are part of every report.
