...
SPEC = importlib.util.spec_from_file_location(
    "panoptes_baseline", ROOT / "baselines" / "baseline.py"
)
...
def finalize_run(workspace: Path, contributor: str | None = None):
    ...
    if (
        contributor
        and baseline.main(["submit", "--run", str(run_dir), "--contributor", contributor]) != 0
    ):
...
def test_prompt_manifest_validates_against_schema():
    errors = validate_submission.validate_file(
        ROOT / "baselines" / "prompts" / "prompts.manifest.json"
    )
    assert errors == []
...
def test_scaffold_finalize_submit_verify_flow(workspace):
    ...
    assert baseline.main(["finalize", "--run", str(run_dir), "--interface", "chat-ui"]) == 2
    ...
    assert validate_submission.validate_file(run_dir / "run.manifest.json") == []
...
def test_pending_run_renamed_on_finalize(workspace):
    ...
    assert (
        baseline.main(
            ["finalize", "--run", str(pending), "--model", "agent-x", "--interface", "agent-chat"]
        )
        == 0
    )
...
def test_finalize_requires_declared_model(workspace):
    ...
    assert baseline.main(["finalize", "--run", str(pending), "--interface", "agent-chat"]) == 2
...
def test_resolve_watermark_anthropic_post_transition_defaults_to_suspected():
    block = baseline.resolve_watermark(
        provider="anthropic",
        created_utc="2026-08-15T00:00:00Z",
    )
    assert block["status"] == "suspected"
    assert block["scheme"] == "synthid-text"


def test_resolve_watermark_explicit_override_wins():
    block = baseline.resolve_watermark(
        provider="anthropic",
        created_utc="2026-08-15T00:00:00Z",
        status="declared-none",
        notes="pre-watermark snapshot",
    )
    assert block == {"status": "declared-none", "notes": "pre-watermark snapshot"}


def test_resolve_watermark_non_anthropic_is_unknown():
    block = baseline.resolve_watermark(
        provider="openai",
        created_utc="2026-08-15T00:00:00Z",
    )
...
def test_finalize_records_explicit_watermark(workspace):
    assert baseline.main(["scaffold", "--model", "test-model", "--kind", "text"]) == 0
    run_dir = workspace / "runs" / "test-model_text"
    fill_outputs(run_dir, "text")
    assert (
    ...
    manifest = json.loads((run_dir / "run.manifest.json").read_text(encoding="utf-8"))
    assert manifest["watermark"]["status"] == "declared-active"
    assert manifest["watermark"]["scheme"] == "synthid-text"
    assert validate_submission.validate_file(run_dir / "run.manifest.json") == []


def test_finalize_anthropic_heuristic_suspected(workspace):
    assert baseline.main(["scaffold", "--model", "claude-test", "--kind", "text"]) == 0
    run_dir = workspace / "runs" / "claude-test_text"
    fill_outputs(run_dir, "text")
    assert (
        baseline.main(
            [
    ...
    manifest = json.loads((run_dir / "run.manifest.json").read_text(encoding="utf-8"))
    assert manifest["watermark"]["status"] == "suspected"
    assert validate_submission.validate_file(run_dir / "run.manifest.json") == []
